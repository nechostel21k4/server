...
exports.uploadImage = async (req, res) => {
  try {
    ...
    // ✅ SECURITY: Ensure student can only upload their own image
    if (req.user && req.user.role === 'student' && req.user.rollNo !== username) {
      return res.status(403).json({ imageUploaded: false, message: "Forbidden: You can only update your own profile picture." });
    }
